...
export default {
  ...
  methods: {
      createChart() {
          ...
          const flattenData = (data, categories) => {
              const result = [];
              data[0].forEach((d, i) => {
                  const item = { label: d.label };
                  categories.forEach((category, k) => {
                      item[category] = {
                          value: data[k][i].value,
          ...
          const flattenedData = flattenData(this.data, this.categories);

          console.log("flattened", flattenedData);

          x0.domain(flattenedData.map((d) => d.label));
          x1.domain(this.categories).rangeRound([0, x0.bandwidth()]);
          y.domain([
              0,
              d3.max(flattenedData, (d) =>
                  d3.max(this.categories, (category) => d[category].value)
              ),
          ]).nice();
          ...
          svg.append("g")
              ...
              .selectAll("rect")
              .data((d) => {
                  const mappedData = this.categories.map((category) => ({
                      category,
                      value: d[category].value,
                      percentage: d[category].percentage,
                      label: d.label,
                  }));
                  console.log(mappedData); // Tambahkan log ini untuk melihat hasil map()
                  return mappedData;
              })
              .enter()
              .append("rect")
          ...
          svg.append("g")
              ...
              .attr("fill", "black")
              .style("opacity", 0)
              .text((d) => (this.isPercentage ? `${d.percentage}%` : d.value))
